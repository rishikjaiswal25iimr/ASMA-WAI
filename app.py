...
from src import (
    data_loader, preprocessing, eda, nlp_analysis, feature_engineering,
    modeling, evaluation, explainability, prediction, optimization, recommendations,
)
...
def _load(file_bytes: bytes, filename: str) -> pd.DataFrame:
    return data_loader.load_dataset(io.BytesIO(file_bytes) if not filename.endswith((".xlsx", ".xls"))
...
def load_default_or_uploaded(uploaded_file, default_path: str, label: str) -> pd.DataFrame:
    if uploaded_file is not None:
        ...
        df = data_loader.load_dataset(default_path)
...
def run_data_pipeline(train_df: pd.DataFrame, pred_df: pd.DataFrame):
    ...
    mapping = data_loader.build_column_mapping(train_df)

    train_clean, train_clean_report = preprocessing.clean_dataset(train_df, mapping)
    pred_clean, pred_clean_report = preprocessing.clean_dataset(pred_df, mapping)
    ...
    if eng_col and eng_col in train_fe.columns:
        train_fe, (pred_fe,), threshold = feature_engineering.build_target(
            train_fe, [pred_fe], eng_col, percentile=TARGET_PERCENTILE
        )

    feature_cols = feature_engineering.get_pre_publication_feature_columns(train_fe, mapping)
    leakage_table = feature_engineering.classify_variable_roles(train_fe.columns.tolist(), mapping)
...
def run_modeling_pipeline(train_fe: pd.DataFrame, feature_cols: tuple, model_names: tuple):
    ...
    results = modeling.train_all_models(X_train, y_train, model_names=list(model_names))
    comparison_table, eval_cache = evaluation.build_comparison_table(results, X_val, y_val)
    best_name = evaluation.select_best_model(comparison_table)

    threshold_table = evaluation.select_threshold(y_val, eval_cache[best_name]["y_proba"])
    best_threshold = evaluation.best_threshold_by_f1(threshold_table)

    final_model, train_secs, best_params = modeling.train_one_model(best_name, X, y)
...
# Run the deterministic data pipeline (fast, always safe to run)
...
# Model pipeline is heavier — it must NEVER run just because the app started.
# It only runs when: (a) the user explicitly clicks "Run / Refresh Full Analysis", OR
# (b) the user has navigated to a page that actually needs model results and no
# cached result exists yet for this session. Pages 1-5 (Overview, Data Quality,
# Descriptive Analytics, NLP, Feature Engineering) never trigger training.
PAGES_REQUIRING_MODEL = {PAGES[5], PAGES[6], PAGES[7], PAGES[8], PAGES[9]}  # 6-10

if "model_output" not in st.session_state:
    st.session_state["model_output"] = None

needs_model_now = page in PAGES_REQUIRING_MODEL
should_train = run_button or (needs_model_now and st.session_state["model_output"] is None)

if should_train:
    if eng_col is not None and model_choices:
        with st.spinner("Training models on the 15,000-record training dataset... this can take ~30-60s."):
            st.session_state["model_output"] = run_modeling_pipeline(
                train_fe, tuple(feature_cols), tuple(model_choices)
            )
    elif eng_col is None:
        st.sidebar.error("Cannot train models — no engagement-rate column was detected.")
...
# ==============================================================================
    ...
    for tab, df, label in [(tab1, train_raw, "Training"), (tab2, pred_raw, "Prediction")]:
        with tab:
            report = data_loader.data_quality_report(df)
            kpi_row([
...
# ==============================================================================
    ...
    if cat_col:
        ...
        summary = eda.group_engagement(df, cat_col, eng_col)
        c1, c2 = st.columns([2, 1])
        c1.plotly_chart(eda.bar_chart(summary, cat_col, "avg_engagement", f"Average Engagement Rate by {cat_col}"),
    ...
    if device_col:
        ...
        summary = eda.group_engagement(df, device_col, eng_col)
        c1, c2 = st.columns([2, 1])
        c1.plotly_chart(eda.bar_chart(summary, device_col, "avg_engagement", f"Average Engagement Rate by {device_col}"),
                         use_container_width=True)
    ...
    if "post_day_of_week" in df.columns:
        ...
        dow_summary = eda.group_engagement(df, "post_day_of_week", eng_col)
        ...
        c1, c2 = st.columns(2)
        c1.plotly_chart(eda.bar_chart(dow_summary, "post_day_of_week", "avg_engagement", "Average Engagement by Day of Week"),
                         use_container_width=True)
        wk_fig, wk_data = eda.weekend_vs_weekday(df, eng_col)
        if wk_fig:
    ...
    if followers_col:
        c1, c2 = st.columns(2)
        c1.plotly_chart(eda.scatter_plot(df.sample(min(3000, len(df)), random_state=42), followers_col, eng_col,
                                          f"{followers_col} vs. {eng_col}", trendline="lowess"), use_container_width=True)
        if "follower_band" in df.columns:
            band_summary = eda.group_engagement(df, "follower_band", eng_col)
            c2.plotly_chart(eda.bar_chart(band_summary, "follower_band", "avg_engagement", "Engagement by Follower Tier"),
                             use_container_width=True)

    verified_col = mapping.get("is_verified")
    if verified_col:
        st.markdown("**Verified vs. non-verified accounts**")
        v_summary = eda.group_engagement(df, verified_col, eng_col)
        st.plotly_chart(eda.bar_chart(v_summary, verified_col, "avg_engagement", "Engagement: Verified vs. Non-Verified"),
                         use_container_width=True)
    ...
    if len(numeric_cols) >= 2:
        heat_fig, _ = eda.correlation_heatmap(df, numeric_cols)
        st.plotly_chart(heat_fig, use_container_width=True)

    # --- High vs low distribution ---
    st.markdown("### High vs. Low Engagement Split")
    pie_fig, counts = eda.high_vs_low_distribution(df, "high_engagement")
...
# ==============================================================================
    ...
    with c2:
        ...
        s_summary = eda.group_engagement(df, "sentiment_label", eng_col)
        st.plotly_chart(eda.bar_chart(s_summary, "sentiment_label", "avg_engagement", "Average Engagement by Sentiment"),
                         use_container_width=True)

    cat_col = mapping.get("category")
    if cat_col:
        st.markdown("**Sentiment × Category (mean engagement, top rows)**")
        cross = nlp_analysis.sentiment_vs_dimension(df, "sentiment_label", cat_col, eng_col)
    ...
    if device_col:
        ...
        cross2 = nlp_analysis.sentiment_vs_dimension(df, "sentiment_label", device_col, eng_col)
    ...
    with c1:
        ...
        st.dataframe(nlp_analysis.top_ngrams(df.loc[df["high_engagement"] == 1, text_col], n=1, top_k=15),
                     use_container_width=True)
    with c2:
        st.caption("Low engagement")
        st.dataframe(nlp_analysis.top_ngrams(df.loc[df["high_engagement"] == 0, text_col], n=1, top_k=15),
                     use_container_width=True)
    st.caption("Frequency-based extraction (stopwords removed); this is descriptive vocabulary analysis, not topic modeling.")
...
# ==============================================================================
    ...
    eng_added = [c for c in feature_engineering.ENGINEERED_FEATURE_COLUMNS if c in train_fe.columns]
    st.write(eng_added)
...
training dataset, using an internal **80:20 stratified train/validation split** and light
`GridSearchCV` hyperparameter tuning (3-fold CV, optimizing F1):
...
- **SVM** — classical margin-based classifier (trained on a stratified 6,000-row subsample when the
...
""")
    ...
    c1, c2 = st.columns(2)
    c1.plotly_chart(evaluation.roc_curve_figure(model_output["eval_cache"], model_output["y_val"]),
                     use_container_width=True)
    with c2:
        ...
        st.plotly_chart(evaluation.confusion_matrix_figure(cm, chosen), use_container_width=True)
...
# ==============================================================================
    ...
    if best_name in ("Random Forest", "XGBoost"):
        ...
        with st.spinner("Computing SHAP values..."):
            shap_values, feat_names, X_trans, X_samp = explainability.explain_tree_model(final_model, X_val, max_rows=400)
        ...
        single = explainability.explain_single_prediction(shap_values, feat_names, row_idx)
        ...
        coef_table = explainability.logistic_coefficient_importance(final_model) if best_name == "Logistic Regression" else None
...
# ==============================================================================
    ...
    pred_out = prediction.predict_on_new_data(final_model, X_pred, best_threshold)
    final_output = prediction.assemble_prediction_output(pred_raw, pred_fe, pred_out)
    ...
    if "high_engagement" in pred_fe.columns:
        ...
        oos = prediction.evaluate_out_of_sample(pred_fe["high_engagement"], pred_out["predicted_high_engagement"],
        ...
        st.plotly_chart(evaluation.confusion_matrix_figure(oos["confusion_matrix"], f"{model_output['best_name']} — Out-of-Sample"),
                         use_container_width=True)
    else:
...
""")
    st.caption(f"Non-controllable variables held fixed: {optimization.CONTEXTUAL_FIELDS_NOTE}")

    final_model = model_output["final_model"]
    with st.spinner("Scoring content scenarios..."):
        scenario_df = optimization.generate_scenarios(train_fe, feature_cols, max_combinations=300)
...
# ==============================================================================
    ...
    if model_output is None:
        st.warning("Click **Run / Refresh Full Analysis** in the sidebar for model-based recommendations.")

    cards = []
    cat_col = mapping.get("category")
    if cat_col:
        cat_summary = eda.group_engagement(train_fe, cat_col, eng_col)
        cards.append(recommendations.category_insight(cat_summary, cat_col))

    device_col = mapping.get("device")
    if device_col:
        device_summary = eda.group_engagement(train_fe, device_col, eng_col)
        cards.append(recommendations.device_insight(device_summary, device_col))

    if "sentiment_label" in train_fe.columns:
        sentiment_summary = eda.group_engagement(train_fe, "sentiment_label", eng_col).rename(
            columns={"sentiment_label": "sentiment_label", "avg_engagement": "avg_engagement"})
        cards.append(recommendations.sentiment_insight(sentiment_summary))

    if "is_weekend" in train_fe.columns:
        wk_fig, wk_data = eda.weekend_vs_weekday(train_fe, eng_col)
        if wk_data is not None:
            cards.append(recommendations.timing_insight(wk_data))

    if model_output is not None:
        best_row = model_output["comparison_table"].iloc[0]
        cards.append(recommendations.model_insight(model_output["best_name"], best_row))
...
- Instrument true multi-platform tracking (this dataset lacks a network-level platform field)
  and time-of-day posting data to unlock richer optimization.
