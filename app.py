...
@st.cache_data(show_spinner=False)
def run_train_pipeline(train_df: pd.DataFrame):
    """Mapping -> cleaning -> feature engineering -> target construction, TRAINING DATA ONLY.
    Cached on the training data's content. This deliberately never touches the prediction
    dataset (see run_prediction_pipeline below), so Pages 1-7 and app startup never clean,
    engineer features for, or compute a target on the 5,000 held-out records."""
    ...
    if eng_col and eng_col in train_fe.columns:
        train_fe, _, threshold = core.build_target(
            train_fe, [], eng_col, percentile=TARGET_PERCENTILE
        )
    ...
    return {
        "mapping": mapping,
        "train_clean": train_clean, "train_clean_report": train_clean_report,
        "train_fe": train_fe,
...
@st.cache_data(show_spinner=False)
def run_prediction_pipeline(pred_df: pd.DataFrame, mapping: dict, train_fe: pd.DataFrame, eng_col: str):
    """Cleaning -> feature engineering -> target construction for the held-out PREDICTION DATA
    ONLY, reusing the column mapping already derived from training. Called ONLY from the Page 8
    / Prediction workflow below — never at startup and never from Pages 1-7 — so the 5,000
    held-out records stay genuinely untouched until the user actually reaches that page. The
    threshold applied is recomputed from `train_fe` alone (same 75th-percentile rule as during
    training), so this never uses the prediction data to derive the threshold."""
    pred_clean, pred_clean_report = core.clean_dataset(pred_df, mapping)
    pred_fe = core.engineer_features(pred_clean, mapping)

    if eng_col and eng_col in pred_fe.columns:
        _, (pred_fe,), _ = core.build_target(
            train_fe, [pred_fe], eng_col, percentile=TARGET_PERCENTILE
        )

    return {"pred_clean": pred_clean, "pred_clean_report": pred_clean_report, "pred_fe": pred_fe}
...
def run_modeling_pipeline(train_fe: pd.DataFrame, feature_cols: tuple, model_names: tuple,
    ...
    with st.spinner(f"Refitting {best_name} on the full 5,000-record training set…"):
...
train_upload = st.sidebar.file_uploader("Training dataset (5,000 records)", type=["csv", "xlsx", "xls"])
pred_upload = st.sidebar.file_uploader("Prediction dataset (2,000 unseen records)", type=["csv", "xlsx", "xls"])
...
# NOTE: the prediction dataset (data/prediction_data.csv) is intentionally NOT loaded here.
# It is only loaded — and only cleaned/feature-engineered — inside the specific page sections
# that actually need it (Page 1's raw snapshot, Page 2's raw quality report, and Page 8's
# prediction workflow), so it is never processed at startup or on Pages 3-7, 9, 10.
...
# ------------------------------------------------------------------------------------------
# EVERYTHING analytical — mapping, cleaning, NLP/feature engineering, target construction,
# AND model training — happens ONLY inside this `if run_button:` block. Nothing above this
# line does anything more than render widgets and read the raw training file's bytes into a
# DataFrame (a lightweight, necessary read — no cleaning, no NLP, no feature engineering).
# On a fresh app start, run_button is False, so this entire block is skipped and the script
# falls straight through to the "not yet processed" landing view a few lines down: the UI
# renders immediately, with zero cleaning/NLP/feature engineering/target construction/model
# training/SHAP/prediction having executed.
# ------------------------------------------------------------------------------------------
...
if run_button:
    with st.spinner("Preparing training data (column mapping, cleaning, feature engineering)… "
                     "first run on new data can take a little while, later runs are cached."):
        st.session_state["pipeline_output"] = run_train_pipeline(train_raw)

    _pipeline = st.session_state["pipeline_output"]
    if _pipeline["eng_col"] is None:
        st.sidebar.error("Could not detect an engagement-rate column — high-engagement target cannot be built.")

    # Model training — still runs only as part of this same explicit click, never on its own.
    # Each model within it is still cached individually (see run_modeling_pipeline /
    # _train_single_model_cached above), so repeated explicit runs after changing only one
    # model selection don't retrain the others from scratch.
    if _pipeline["eng_col"] is not None and model_choices:
        st.session_state["model_output"] = run_modeling_pipeline(
            _pipeline["train_fe"], tuple(_pipeline["feature_cols"]), tuple(model_choices),
            enable_tuning, cv_folds, svm_subsample,
        )
        st.session_state["model_run"] = True
    else:
        st.sidebar.warning("Select at least one model (and make sure an engagement-rate column "
                            "was detected) before running the analysis.")

pipeline = st.session_state.get("pipeline_output")
model_output = st.session_state.get("model_output")

if pipeline is None:
    # Nothing has been processed yet — this is the lightweight startup/landing state. No
    # training-data cleaning, NLP, feature engineering, target construction, model training,
    # SHAP, optimization, or prediction has run at this point.
    ...
    st.info("👋 **Nothing has been processed yet.** Click **🚀 Run / Refresh Full Analysis** in the "
            "sidebar to clean and feature-engineer the training data and train the models. The app "
            "starts up instantly and performs no cleaning, NLP, feature engineering, target "
            "construction, model training, SHAP, or prediction until you click that button.")
    st.caption(f"Training file ready to process: **{len(train_raw):,} records** "
               f"({'uploaded file' if train_upload is not None else 'default project file'}).")
    st.stop()

mapping = pipeline["mapping"]
train_fe = pipeline["train_fe"]
...
**Data split (Section 5).** The 7,000-record dataset is split into the first **5,000 records**
(training/model development) and the last **2,000 records** (completely unseen prediction set).
...
""")
    ...
    st.caption("The held-out prediction dataset (5,000 unseen records) is not loaded or processed anywhere "
               "in the app until you reach **Page 8 — Prediction**.")
    ...
    st.write("**Training data (first 5,000 rows)**")
    st.dataframe(train_fe.head(5), use_container_width=True)
    st.caption("The prediction data snapshot is shown on Page 8 once it's loaded and processed there.")
...
# ==============================================================================
    ...
    with tab1:
        ...
        st.markdown("**Training data — column-level quality report**")
        st.dataframe(report["missing_table"], use_container_width=True)
        st.markdown("**Descriptive statistics (numeric columns)**")
        st.dataframe(train_raw.describe().T, use_container_width=True)
    with tab2:
        st.info("The prediction dataset (`data/prediction_data.csv`) is not loaded anywhere in the app "
                "until you reach **Page 8 — Prediction**, so its quality report isn't available here. "
                "Visit Page 8 to load it, generate predictions, and (if ground-truth labels exist) see "
                "its out-of-sample evaluation.")
...
# ==============================================================================
    ...
    st.caption("Computed on the 5,000-record TRAINING dataset only.")
...
# ==============================================================================
    ...
    if threshold is not None:
        ...
        st.caption("The prediction dataset's high-engagement rate (if ground-truth labels exist there) is "
                   "shown on **Page 8 — Prediction**, computed only after predictions are generated. The "
                   "held-out 5,000 records are not cleaned, feature-engineered, or labelled anywhere before "
                   "that page — they stay genuinely unseen until then (Section 5/18).")
    else:
...
# ==============================================================================
    ...
    st.markdown(f"""
Four classifiers are trained **independently** (none feeds another) on the same 5,000-record
...
# ==============================================================================
    ...
    st.info(f"Applying the finalized **{model_output['best_name']}** model (retrained on the full 5,000-record "
            f"training set, threshold = {best_threshold:.2f}) to the **2,000-record prediction dataset**, which "
            f"was never touched during model development.")

    # This is the ONLY point in the app where the held-out prediction dataset is loaded,
    # cleaned, feature-engineered and (for transparency only, AFTER scoring) labelled. It is
    # never used for feature selection, training, tuning, threshold selection, or SHAP.
    with st.spinner("Loading and preparing the held-out prediction dataset…"):
        pred_raw = load_default_or_uploaded(pred_upload, DEFAULT_PRED_PATH, "Prediction data")
        pred_pipeline = run_prediction_pipeline(pred_raw, mapping, train_fe, eng_col)
    pred_fe = pred_pipeline["pred_fe"]
