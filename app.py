...
# Run the deterministic (non-ML) data pipeline — TRAINING DATA ONLY. This is CPU-light on
# repeat runs because engineer_features() is disk-cached (joblib.Memory) inside core.py — the
# slow, one-time cost (TextBlob sentiment over every caption) only happens once per unique
# dataset, including across app restarts. This is the only pipeline work done at startup; it
# renders the dashboard immediately without training any models or touching prediction data.
with st.spinner("Preparing training data (column mapping, cleaning, feature engineering)… "
    ...
    pipeline = run_train_pipeline(train_raw)
...
# Model training NEVER happens automatically — not at startup, and not just because a page
# (Model Training, SHAP, Prediction, etc.) was opened. It runs ONLY when the user explicitly
# clicks "Run / Refresh Full Analysis" above. Each model within it is still cached individually
# (see run_modeling_pipeline / _train_single_model_cached above), so repeated explicit runs
# after changing only one model selection don't retrain the others from scratch.
...
""")
    ...
    # Loaded here, lazily, and used RAW only (no cleaning/feature engineering/target) — just to
    # report a record count and a snapshot. Full processing happens only on Page 8.
    pred_raw_snapshot = load_default_or_uploaded(pred_upload, DEFAULT_PRED_PATH, "Prediction data")

    kpi_row([
        ("Training records", f"{len(train_fe):,}"),
        ("Prediction records (unseen)", f"{len(pred_raw_snapshot):,}"),
    ...
    with c2:
        st.write("**Prediction data (last 5,000 rows — unseen, raw)**")
        st.caption("Shown raw/unprocessed here — the prediction set is only cleaned, "
                   "feature-engineered and scored on Page 8, so it stays genuinely held out until then.")
        st.dataframe(pred_raw_snapshot.head(5), use_container_width=True)
...
# ==============================================================================
    ...
    # Loaded here, lazily, and used RAW only (no cleaning/feature engineering) — the quality
    # report below inspects raw structure/missing values only. Full processing happens on Page 8.
